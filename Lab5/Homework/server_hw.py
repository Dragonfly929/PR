...
def handle_image_upload(client_socket, image_name, image_data):
    try:
...
def handle_image_download(client_socket, image_name):
    try:
        room_folder = os.path.join("SERVER_MEDIA", connected_clients[client_socket]["room"])
        image_path = os.path.join(room_folder, image_name)
        ...
        if os.path.exists(image_path):
            ...
            client_socket.send(f"The {image_name} doesn't exist".encode('utf-8'))
    ...
    try:
        ...
        while True:
            ...
            if message_type == "message":
                ...
                handle_image_upload(client_socket, image_name, image_data)
            elif message_type == "download":
                image_name = received_message["payload"]["image_name"]
                handle_image_download(client_socket, image_name)
